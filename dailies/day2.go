...
func solve(meal_cost float64, tip_percent int32, tax_percent int32) {
	...
	//var taxf64 float64 = float64(tax_percent) / 100
	var taxedMealCost = (meal_cost * (float64(tax_percent) / 100)) + meal_cost
	var taxedWithtipCost = math.Round((float64(tip_percent) / 100 * meal_cost) + taxedMealCost)
	fmt.Printf("%v", taxedWithtipCost)
